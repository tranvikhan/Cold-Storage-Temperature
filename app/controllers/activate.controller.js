...
exports.Activate = (req,res) =>{
    ...
    .then(user =>{
        axios
        .get(config.real.baseURL+config.real.api.getStation+user.data.user_id,
            { 'headers': 
                { 'Authorization': 'JWT '+ user.data.accessToken} 
            }
        )
        .then(station =>{
            if(station.data.data.length>0){
                let stations = station.data.data.map(stationX => ({station_id:stationX.station_id,station_name:stationX.station_name}))
...
exports.addActivate = (req,res) =>{
    ...
    .then(user =>{
        Activate
        .findOne({room:req.body.room_id, station_id: req.body.station_id})
        .exec((err,activate)=>{
            if(err){
                result.ServerError(res, err);
            }
            if(!activate){
                axios
                .get(config.real.baseURL+config.real.api.getValue+req.body.station_id,
                    { 'headers': 
                        { 'Authorization': 'JWT '+ user.data.accessToken} 
                    }
                )
...
exports.removeActivate = (req,res) =>{
    Structure.findOne({ room: req.body.room_id}).sort({"createdAt":-1}).exec((err,Oldstructure)=>{
        ...
        Activate.find({room: req.body.room_id,_id: req.body.activate_id}).exec((err,activates)=>{
            ...
            if(activates){
                Sensor.find({activate: {$in: activates.map(at=>(at._id))}, isUsed : true }).exec((err,sensors)=>{
                    if(err){
                        result.ServerError(res,err)
                        return;
                    }
                    if(sensors){
                            ...
                            newStructure.map = newStructure.map.filter(st => {
                                return sensors.find(sr =>(sr._id+'' == st.sensor+'')) ==null;
                            });
